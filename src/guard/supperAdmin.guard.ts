...
export class SuperAdminGuard implements CanActivate {
  ...
  canActivate(
    ...
    try {
      ...
      if (!token) {
        this.logger.warn(
          'SuperAdminGuard failed: No authorization token provided',
        );
        return false;
      }
      ...
      if (decode.role !== 'SuperAdmin') {
        this.logger.warn(
          `SuperAdminGuard failed: Insufficient role. Required: SuperAdmin, Got: ${decode.role}`,
        );
        return false;
      }

      this.logger.log(
        `SuperAdminGuard successful: User ${decode.email} with role ${decode.role} authorized`,
      );
      req.user = decode;
      return true;
    } catch (error) {
